...
const OrderCancelledScreen = ({ navigation }) => {
    ...
    const fetchOrders = async (currentPage) => {
        ...
        const res = await callFetchOrdersByStatus('Đã hủy', currentPage, size);
        if (res && res.result) {
    ...
    return (
        <View className="flex-1 bg-white">
            <FlatList
                ...
                keyExtractor={(item, index) => `cancelled_orders_${item.id}_${index}`}
